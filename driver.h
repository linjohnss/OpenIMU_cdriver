...
int16_t concat_16(int8_t a, int8_t b) { return a << 8 | (b & 0x00ff); }
        ...
        if (head == header) {
          if (read(serial_port, &p_type, sizeof(p_type)) > 0) {
            if (read(serial_port, &length, sizeof(length)) > 0) {
              int16_t *buffer =
                  (int16_t *)malloc((length / 2) * sizeof(int16_t));
              if (read(serial_port, &(*buffer),
                       (length / 2) * sizeof(int16_t)) > 0) {
                return buffer;
              }
            }
          }
          ...
          if (read(serial_port, &p_type, sizeof(p_type)) > 0) {
            if (p_type == packet_type) {
              if (read(serial_port, &length, sizeof(length)) > 0) {
                ...
                if (read(serial_port, &(*data), (length + 2) * sizeof(int8_t)) >
                    0) {
                  int8_t *buffer =
                      (int8_t *)malloc((length + 3) * sizeof(int8_t));
                  ...
                  if (CalculateCRC(buffer, length + 3) ==
                      (uint16_t)concat_16(data[length + 1], data[length]))
                    return data;
                }
              }
            }
